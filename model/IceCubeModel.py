...
class IceCubeModel(Model):
    ...
    def training_step(self, train_batch: Data, batch_idx: int) -> Tensor:
        ...
        meanAngularError = self.compute_metric(preds, train_batch)
    ...
    def compute_metric(self, preds: Tensor, data: Data):
        target = data['direction']
        preds = preds[0]
        preds = preds[:,0:3] * preds[:,3].unsqueeze(1)
        AngularError = torch.arccos(target[:,0] * preds[:,0] + target[:,1] * preds[:,1] + target[:,2] * preds[:,2])
        return torch.mean(AngularError)
    ...
    def validation_step(self, val_batch: Data, batch_idx: int) -> Tensor:
        ...
        meanAngularError = self.compute_metric(preds, val_batch)
    ...
    def compute_loss(self, preds: Tensor, data: Data, verbose: bool = False) -> Tensor:
